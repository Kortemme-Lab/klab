...
def test_pdbml_speed():
    ...
    for test_case in test_cases:
        ...
        try:
            assert(list(p_minidom.atom_to_seqres_sequence_maps.keys()) == list(p_sax.atom_to_seqres_sequence_maps.keys()))
            for c, s_1 in p_minidom.atom_to_seqres_sequence_maps.items():
...
def test_ResidueRelatrix_1A2C():
    ...
    uniparc_id_1 = rr.convert(chain_id, seqres_id_1, 'seqres', 'uniparc')
    assert(uniparc_id_1 == ('UPI0000136ECD', 484))
...
def test_ResidueRelatrix_1A2P():
    ...
    uniparc_id_1 = rr.convert('A', seqres_id_1, 'seqres', 'uniparc')
    assert(uniparc_id_1 == ('UPI000013432A', 128))

    # Double/triple jumps forward
    seqres_id_2 = rr.convert('A', rosetta_id_1, 'rosetta', 'seqres')
    assert(seqres_id_2 == 81)
    uniparc_id_2 = rr.convert('A', rosetta_id_1, 'rosetta', 'uniparc')
    assert(uniparc_id_2 == ('UPI000013432A', 128))

    # Double jumps forward
    uniparc_id_3 = rr.convert('A', atom_id_1, 'atom', 'uniparc')
    assert(uniparc_id_3 == ('UPI000013432A', 128))
    ...
    assert(rr.convert_from_rosetta(3, 'uniparc') == ('UPI000013432A', 52))
...
for c, m in sorted(sifts_map.atom_to_uniparc_sequence_maps.items()):
    colortext.message(c)
    print(m)
...
for uniparc_id, uniparc_sequence in uniparc_sequences.items():
...
for c in chains:
    ...
    sequences = [fasta_sequence] + list(uniparc_sequences.values())
    assert(len(uniparc_sequences) == 2)

    colortext.warning("Trying to match chain %c against UniParc IDs %s" % (c, list(uniparc_sequences.keys())))
...
for k, v in sorted(iter(px.atom_to_seqres_sequence_maps.items()), key=lambda x:(x[0], x[1])):
    print((k,v))
...
for pdb_id, details in sorted(refs.items()):
    print(pdb_id)
    for db, chain_details in sorted(details.items()):
        print(("  %s" % db))
        for chain_id, subdetails in sorted(chain_details.items()):
            print(("    Chain %s" % chain_id))
            for k, v in sorted(subdetails.items()):
...
for k,v in p.get_journal().items():
