...
pdb_sequence <- function(pdbpath) {
	...
	# Replace leading spaces with underscores. (todo: Why?)
	resID <- gsub("^ ", "_", resID)
	...
	# Return an array of residue names whose residue IDs are unique
...
process_seqtol <- function(dirpath = ".", fitness_coef = c(1/2.5, 1/2.5, 1/2.5, 1),
	...
	if (file.exists(seqtoloutput)) {
		...
		if (length(seqtolcmd)) {
			...
			startpdbfile <- gsub("^.+/(.*)$", "\\1", startpdbfile, perl=TRUE)
			startpdbfile <- file.path(dirpath, startpdbfile)
			...
			if (file.exists(startpdbfile)) {
				...
				pdbseq <- pdb_sequence(startpdbfile)
	...
	# Write pwm to a tab-separated file (do not including quotation marks)
