...
class SGEConnection(StatusPrinter):
	...
	def qsub_submit(self, command_filename, workingdir, dbID, hold_jobid = None, name = None, showstdout = False, timeInMinutes = None):
		...
		queue = "any"
		if timeInMinutes and timeInMinutes <= 30:
			command.append('-q')
			command.append('short.q')
			queue = "short"
		else:
			command.append('-q')
			command.append('lab.q')
			queue = "lab"
		if hold_jobid:
