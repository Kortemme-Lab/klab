...
class TaskScheduler(object):
    ...
    def __init__(self, workingdir):
        ...
        self.dbID = 0
        self.debug = True
        # todo: The task states here have different meanings to those of ddGClusterTask. Maybe separate them entirely.
        self.tasks = {ddGClusterTask.INITIAL_TASK: [],
                      ddGClusterTask.ACTIVE_TASK: [],
                      ddGClusterTask.RETIRED_TASK: [],
                      ddGClusterTask.COMPLETED_TASK: []}
    ...
    #todo: Use multiple inheritance for this and ddGClusterTask and SGEConnection classes    
    def _status(self, message, plain = False):
        if self.debug:
            ...
            if plain:
                ...
                print('<debug id="%s" type="scheduler" time="%s">%s</debug>' % (self.dbID, timestamp, message))
    ...
    def _movequeue(self, task, oldqueue, newqueue):
        ...
        self._status("Moving %d (%s) from %s to %s" % (task.jobid, task.getName(), ddGClusterTask.status.get(oldqueue), ddGClusterTask.status.get(newqueue)))
        self.tasks[oldqueue].remove(task)
        self.tasks[newqueue].append(task)
        
    def _getAllTasks(self):
        tasks = []
        tasks.extend(self.tasks[ddGClusterTask.INITIAL_TASK])
        tasks.extend(self.tasks[ddGClusterTask.ACTIVE_TASK])
        tasks.extend(self.tasks[ddGClusterTask.RETIRED_TASK])
        tasks.extend(self.tasks[ddGClusterTask.COMPLETED_TASK])
        return tasks        
    ...
    def getprofile(self):
        ...
        for task in self.tasks_in_order:
            status = ddGClusterTask.status.get(task.getState(allowToRetire = False))
    ...
    def addInitialTasks(self, *tasks):
        if self.started:
            raise SchedulerTaskAddedAfterStartException(self.pendingtasks, self.tasks[ddGClusterTask.RETIRED_TASK], self.tasks[ddGClusterTask.COMPLETED_TASK], msg = "Exception adding initial task: %s" % task.getName())
        for task in tasks:
            self._initialtasks.append(task)
            self.tasks[ddGClusterTask.INITIAL_TASK].append(task)
    ...
    def start(self, sgec, dbID):
        ...
        self.sgec = sgec
        self.dbID = dbID
        checkGraphReachability(self.tasks[ddGClusterTask.INITIAL_TASK])
            
        tasksToStart = []
        for task in self.tasks[ddGClusterTask.INITIAL_TASK]:
        ...
        for task in tasksToStart:
            ...
            if started:
                ...
                self._movequeue(task, ddGClusterTask.INITIAL_TASK, ddGClusterTask.ACTIVE_TASK)    
            else:
                self.raiseFailure(SchedulerStartException(self.pendingtasks, self.tasks[ddGClusterTask.RETIRED_TASK], self.tasks[ddGClusterTask.COMPLETED_TASK], msg = "Exception starting: %s" % task.getName()))
                return False
    ...
    def step(self):
        ...
        if self.failed:
             raise BadSchedulerException(self.pendingtasks, self.tasks[ddGClusterTask.RETIRED_TASK], self.tasks[ddGClusterTask.COMPLETED_TASK], msg = "The scheduler has failed.") 
                              
        # Retire tasks
        activeTasks = self.tasks[ddGClusterTask.ACTIVE_TASK][:]
        for task in activeTasks:
            tstate = task.getState()
            if tstate == ddGClusterTask.RETIRED_TASK:
                self._movequeue(task, ddGClusterTask.ACTIVE_TASK, ddGClusterTask.RETIRED_TASK)
            elif tstate == ddGClusterTask.FAILED_TASK:
                self._movequeue(task, ddGClusterTask.ACTIVE_TASK, ddGClusterTask.RETIRED_TASK)
                self.raiseFailure(TaskCompletionException(self.pendingtasks, self.tasks[ddGClusterTask.RETIRED_TASK], self.tasks[ddGClusterTask.COMPLETED_TASK], msg = "The task %s failed." % task.getName()))
                return False
        
        # Complete tasks
        retiredTasks = self.tasks[ddGClusterTask.RETIRED_TASK][:]
        for task in retiredTasks:
            dependents = task.getDependents()
            completed = True
            for dependent in dependents:
                if dependent.getState() != ddGClusterTask.COMPLETED_TASK:
                    completed = False
                    break
            if completed:
                completed = task.complete()
                if completed:
                    self._movequeue(task, ddGClusterTask.RETIRED_TASK, ddGClusterTask.COMPLETED_TASK)
                else:
                    self.raiseFailure(TaskCompletionException(self.pendingtasks, self.tasks[ddGClusterTask.RETIRED_TASK], self.tasks[ddGClusterTask.COMPLETED_TASK], msg = "The task %s failed." % task.getName()))
            ...
            for dependent in dependents:
                ...
                if dependent.getState() == ddGClusterTask.INACTIVE_TASK:
                    ...
                    if started:
                        ...
                        self.tasks[ddGClusterTask.ACTIVE_TASK].append(dependent)

        
        # Bad scheduler. No biscuit.
        if self.tasks[ddGClusterTask.INITIAL_TASK]:
            raise BadSchedulerException(self.pendingtasks, self.tasks[ddGClusterTask.RETIRED_TASK], self.tasks[ddGClusterTask.COMPLETED_TASK], msg = "The scheduler has failed.")  
        ...
        if self.pendingtasks and not (self.tasks[ddGClusterTask.ACTIVE_TASK]):
             raise SchedulerDeadlockException(self.pendingtasks, self.tasks[ddGClusterTask.RETIRED_TASK], self.tasks[ddGClusterTask.COMPLETED_TASK])
                                    
        # We are finished when there are no active or retired tasks
        return not(self.tasks[ddGClusterTask.ACTIVE_TASK] or self.tasks[ddGClusterTask.RETIRED_TASK]) 
...
class ClusterBatchJob(object):
    ...
    def _status(self, message):
        if self.debug:
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print('<debug id="%s" type="job" time="%s">%s</debug>' % (self.jobID, timestamp, message))
    ...
    def isCompleted(self):
        if not self.failed:
            if self.scheduler.step():
                ...
                if not(self.scheduler.tasks[ddGClusterTask.ACTIVE_TASK]):
                    if self.scheduler.step():
                        self.scheduler.cleanup()
                        return True
            return False
