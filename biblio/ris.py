...
class RIS(object):
    ...
    def get_author_names_in_short_format(self):
        ...
        for author in self.authors:
            short.append(RIS.shortFormatAuthor(author))
        return short
    ...
    def parse(self):
        ...
        if not d['date']:
            ...
            try:
                pubdate = d['date']
                if pubdate.endswith('/print'):
                    pubdate = pubdate[:-6]
                tokens = map(int, [t for t in pubdate.split("/") if t])
...
class PublicationEntry(dict):
    ...
    def get_HTML(self, showyear = True):
        d = self.dict
        if showyear:
            d["year_str"] = ", %s" % d["year"]
        else:
    ...
    def __repr__(self):
        ...
        # todo: add missing 'elif d.get("URL")' clause to match get_HTML above
        else:
            return '''%(authors_str)s. %(title)s. %(journal)s %(entry)s%(year)s.''' % d



# todo: Fix this up. The code below is pasted directly from the website script.
def parsePublist(saveRis = True):
    ...
    for i in range(len(risfile)):
        if risfile[i].startswith("ER"):
            risfile = risfile[i+1:]
            break
    risfile = "\n".join([line for line in risfile if line[0:2] != "C8" and line[0:2] != "C7"])
    if saveRis:
...
# todo: Fix this up. The code below is pasted directly from the website script.
def check_author_name_variations():
...
test_ris = u'''
# To add new citations, copy and paste the RIS formatted entry into this file. Citations are grouped. The order in which they appear on the website is first by
# group (group order is determined by the order in which a group's first citation appears in this file) and then by the order in which the citation appears in this
# file.
# There are a few changes needed to make the parsing work:
#    - TY MUST be the first field in a record for the parser.
#    - Add a C8 field indicating the group for the website. This will be stripped from the public RIS file.
#    - Add a C7 field indicating the URL for the Science free download service when applicable. This will be stripped from the public RIS file.
#    - If the DOI is present in an entry, prefix it with "doi: " if not already so. This helps parse the doi which appears in different fields from different sources.
#    - If no DOI is present but the article has a URL, add a UR field. One example of this below is "Computer-aided design of functional protein interactions".
#    - Add the full journal name. This will be abbreviated for the website.
#    - For maximum flexibility, use long form names with middle initials where appropriate. Append a period to abbreviated names e.g. "Kirk, James T."
#    - Make sure the date has the proper format - YYYY/MM/DD/other. The other field can contain other information e.g. a range of days for a conference.
# Any text before the first ER line is ignored for parsing and stripped from the public RIS file.
#
# If the publication is a book chapter (CHAP), use the T2 field for a subtitle when applicable.
ER  -
TY  - JOUR
C8  - 2013 Publications
T1  - Serverification of molecular modeling applications: the Rosetta Online Server that Includes Everyone (ROSIE) [preprint, in press]
JO  - PLoS ONE
PY  - 2013
AU  - Lyskov, Sergey
AU  - Chou, Fang-Chieh
...
AU  - Gray, Jeffrey J.
AU  - Das, Rhiju.
UR  - http://arxiv.org/abs/1302.0029
ER  -
TY  - JOUR
C8  - 2013 Publications
...
TY  - JOUR
C8  - 2012 Publications
C7  - http://www.sciencemag.org/cgi/content/full/336/6083/911?ijkey=RAm8AM.4LYaJU&keytype=ref&siteid=sci
A1  - Eames, Matt
A1  - Kortemme, Tanja
...
N2  - Postsynaptic density 95/discs large/zonus occludens-1 (PDZ) domain?interacting motifs, in addition to their well-established roles in protein scaffolding at the cell surface, are proposed to act as cis-acting determinants directing the molecular sorting of transmembrane cargo from endosomes to the plasma membrane. This hypothesis requires the existence of a specific trans-acting PDZ protein that mediates the proposed sorting operation in the endosome membrane. Here, we show that sorting nexin 27 (SNX27) is required for efficient PDZ-directed recycling of the ?2-adrenoreceptor (?2AR) from early endosomes. SNX27 mediates this sorting function when expressed at endogenous levels, and its recycling activity requires both PDZ domain?dependent recognition of the ?2AR cytoplasmic tail and Phox homology (PX) domain?dependent association with the endosome membrane. These results identify a discrete role of SNX27 in PDZ-directed recycling of a physiologically important signaling receptor, and extend the concept of cargo-specific molecular sorting in the recycling pathway.
ER  -
TY  - JOUR
C8  - 2010 Publications
A1  - Lauck, Florian
...
TY  - JOUR
C8  - 2008 Publications
T1  - A New Twist in TCR Diversity Revealed by a Forbidden ?? TCR
JO  - Journal of Molecular Biology
VL  - 375
IS  - 5
SP  - 1306
...
ER  -
TY  - JOUR
C8  - Prediction and Design of Protein-Protein Interactions
A1  - Kortemme, Tanja
A1  - Baker, David
...
ER  -
TY  - JOUR
C8  - Computational Models & Energy Functions
...
ER  -
TY  - JOUR
C8  - Predictions on Specific Systems
...
TY  - JOUR
C8  - Predictions on Specific Systems
T1  - Contributions of Amino Acid Side Chains to the Kinetics and Thermodynamics of the Bivalent Binding of Protein L to Ig ? Light Chain
...
ER  -
TY  - JOUR
C8  - Peptide Model Systems: Alpha-helices and Beta-sheets
...
C8  - Peptide Model Systems: Alpha-helices and Beta-sheets
A1  - Kortemme, Tanja
A1  - Ram??rez-Alvarado, Marina
A1  - Serrano, Luis
T1  - Design of a 20-Amino Acid, Three-Stranded ?-Sheet Protein
Y1  - 1998/07/10
JF  - Science
JO  - Science
SP  - 253
...
TY  - JOUR
C8  - Peptide Model Systems: Alpha-helices and Beta-sheets
T1  - The design of linear peptides that fold as monomeric ?-sheet structures
JO  - Current Opinion in Structural Biology
VL  - 9
IS  - 4
SP  - 487
...
ER  -
TY  - JOUR
C8  - Electrostatic interactions in Thioredoxin-like Proteins
T1  - Ionisation of Cysteine Residues at the Termini of Model ?-Helical Peptides. Relevance to Unusual Thiol pKa Values in Proteins of the Thioredoxin Family
JO  - Journal of Molecular Biology
VL  - 253
IS  - 5
SP  - 799
...
TY  - JOUR
C8  - LabMember
T1  - Monitoring the Conformational Changes of Photoactivated Rhodopsin from ?icroseconds to Seconds by Transient Fluorescence Spectroscopy?
AU  - Hoersch, Daniel
AU  - Otto, Harald
AU  - Wallat, Ingrid
AU  - Heyn, Maarten P.
